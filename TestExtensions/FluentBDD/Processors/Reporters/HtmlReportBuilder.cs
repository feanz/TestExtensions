	...
	public class HtmlReportBuilder : IReportBuilder
	{
		...
		private void HtmlHead()
		{
			using (OpenTag(HtmlTag.head))
			{
				...
				AddLine(string.Format("<link href='BDD.css' rel='stylesheet'/>"));
				if (_viewModel.UseCustomStylesheet)
					AddLine(string.Format("<link href='BDDCustom.css' rel='stylesheet'/>"));

				AddLine("<script type='text/javascript' src='jquery-1.7.1.min.js'></script>");
				AddLine(string.Format("<script type='text/javascript' src='BDD.js'></script>"));
				if (_viewModel.UseCustomJavascript)
					AddLine(string.Format("<link href='BDDCustom.js' rel='stylesheet'/>"));

				AddLine(string.Format("<title>BDD Test Result {0}</title>", DateTime.Now.ToShortDateString()));
			}
		}
		...
		private void Header()
		{
			...
			{
				AddLine(string.Format("<div id='BDDTitle'>{0}</div>", _viewModel.Configuration.ReportHeader));
				AddLine(string.Format("<div id='BDDDescription'>{0}</div>", _viewModel.Configuration.ReportDescription));
			}
		}
		...
		{
			var tabWidth = _tabCount * TabIndentation;
